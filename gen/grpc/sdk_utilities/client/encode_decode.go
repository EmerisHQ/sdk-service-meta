...
// BuildTxFeeEstimateFunc builds the remote method to invoke for
// "sdk-utilities" service "txFeeEstimate" endpoint.
func BuildTxFeeEstimateFunc(grpccli sdk_utilitiespb.SdkUtilitiesClient, cliopts ...grpc.CallOption) goagrpc.RemoteFunc {
	return func(ctx context.Context, reqpb interface{}, opts ...grpc.CallOption) (interface{}, error) {
		for _, opt := range cliopts {
			opts = append(opts, opt)
		}
		if reqpb != nil {
			return grpccli.TxFeeEstimate(ctx, reqpb.(*sdk_utilitiespb.TxFeeEstimateRequest), opts...)
		}
		return grpccli.TxFeeEstimate(ctx, &sdk_utilitiespb.TxFeeEstimateRequest{}, opts...)
	}
}

// EncodeTxFeeEstimateRequest encodes requests sent to sdk-utilities
// txFeeEstimate endpoint.
func EncodeTxFeeEstimateRequest(ctx context.Context, v interface{}, md *metadata.MD) (interface{}, error) {
	payload, ok := v.(*sdkutilities.TxFeeEstimatePayload)
	if !ok {
		return nil, goagrpc.ErrInvalidType("sdk-utilities", "txFeeEstimate", "*sdkutilities.TxFeeEstimatePayload", v)
	}
	return NewTxFeeEstimateRequest(payload), nil
}

// DecodeTxFeeEstimateResponse decodes responses from the sdk-utilities
// txFeeEstimate endpoint.
func DecodeTxFeeEstimateResponse(ctx context.Context, v interface{}, hdr, trlr metadata.MD) (interface{}, error) {
	message, ok := v.(*sdk_utilitiespb.TxFeeEstimateResponse)
	if !ok {
		return nil, goagrpc.ErrInvalidType("sdk-utilities", "txFeeEstimate", "*sdk_utilitiespb.TxFeeEstimateResponse", v)
	}
	if err := ValidateTxFeeEstimateResponse(message); err != nil {
		return nil, err
	}
	res := NewTxFeeEstimateResult(message)
	return res, nil
}

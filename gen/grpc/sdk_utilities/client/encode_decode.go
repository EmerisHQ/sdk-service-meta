...
// BuildOsmoPoolsFunc builds the remote method to invoke for "sdk-utilities"
// service "osmoPools" endpoint.
func BuildOsmoPoolsFunc(grpccli sdk_utilitiespb.SdkUtilitiesClient, cliopts ...grpc.CallOption) goagrpc.RemoteFunc {
	return func(ctx context.Context, reqpb interface{}, opts ...grpc.CallOption) (interface{}, error) {
		for _, opt := range cliopts {
			opts = append(opts, opt)
		}
		if reqpb != nil {
			return grpccli.OsmoPools(ctx, reqpb.(*sdk_utilitiespb.OsmoPoolsRequest), opts...)
		}
		return grpccli.OsmoPools(ctx, &sdk_utilitiespb.OsmoPoolsRequest{}, opts...)
	}
}

// EncodeOsmoPoolsRequest encodes requests sent to sdk-utilities osmoPools
// endpoint.
func EncodeOsmoPoolsRequest(ctx context.Context, v interface{}, md *metadata.MD) (interface{}, error) {
	payload, ok := v.(*sdkutilities.OsmoPoolsPayload)
	if !ok {
		return nil, goagrpc.ErrInvalidType("sdk-utilities", "osmoPools", "*sdkutilities.OsmoPoolsPayload", v)
	}
	return NewProtoOsmoPoolsRequest(payload), nil
}

// DecodeOsmoPoolsResponse decodes responses from the sdk-utilities osmoPools
// endpoint.
func DecodeOsmoPoolsResponse(ctx context.Context, v interface{}, hdr, trlr metadata.MD) (interface{}, error) {
	message, ok := v.(*sdk_utilitiespb.OsmoPoolsResponse)
	if !ok {
		return nil, goagrpc.ErrInvalidType("sdk-utilities", "osmoPools", "*sdk_utilitiespb.OsmoPoolsResponse", v)
	}
	if err := ValidateOsmoPoolsResponse(message); err != nil {
		return nil, err
	}
	res := NewOsmoPoolsResult(message)
	return res, nil
}

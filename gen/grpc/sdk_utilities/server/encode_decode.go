...
// EncodeTxFeeEstimateResponse encodes responses from the "sdk-utilities"
// service "txFeeEstimate" endpoint.
func EncodeTxFeeEstimateResponse(ctx context.Context, v interface{}, hdr, trlr *metadata.MD) (interface{}, error) {
	result, ok := v.(*sdkutilities.FeeEstimate)
	if !ok {
		return nil, goagrpc.ErrInvalidType("sdk-utilities", "txFeeEstimate", "*sdkutilities.FeeEstimate", v)
	}
	resp := NewTxFeeEstimateResponse(result)
	return resp, nil
}

// DecodeTxFeeEstimateRequest decodes requests sent to "sdk-utilities" service
// "txFeeEstimate" endpoint.
func DecodeTxFeeEstimateRequest(ctx context.Context, v interface{}, md metadata.MD) (interface{}, error) {
	var (
		message *sdk_utilitiespb.TxFeeEstimateRequest
		ok      bool
	)
	{
		if message, ok = v.(*sdk_utilitiespb.TxFeeEstimateRequest); !ok {
			return nil, goagrpc.ErrInvalidType("sdk-utilities", "txFeeEstimate", "*sdk_utilitiespb.TxFeeEstimateRequest", v)
		}
		if err := ValidateTxFeeEstimateRequest(message); err != nil {
			return nil, err
		}
	}
	var payload *sdkutilities.TxFeeEstimatePayload
	{
		payload = NewTxFeeEstimatePayload(message)
	}
	return payload, nil
}

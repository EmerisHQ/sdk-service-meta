...
func New(e *sdkutilities.Endpoints, uh goagrpc.UnaryHandler) *Server {
	return &Server{
		...
		OsmoPoolsH:           NewOsmoPoolsHandler(e.OsmoPools, uh),
...
// NewOsmoPoolsHandler creates a gRPC handler which serves the "sdk-utilities"
// service "osmoPools" endpoint.
func NewOsmoPoolsHandler(endpoint goa.Endpoint, h goagrpc.UnaryHandler) goagrpc.UnaryHandler {
	if h == nil {
		h = goagrpc.NewUnaryHandler(endpoint, DecodeOsmoPoolsRequest, EncodeOsmoPoolsResponse)
	}
	return h
}

// OsmoPools implements the "OsmoPools" method in
// sdk_utilitiespb.SdkUtilitiesServer interface.
func (s *Server) OsmoPools(ctx context.Context, message *sdk_utilitiespb.OsmoPoolsRequest) (*sdk_utilitiespb.OsmoPoolsResponse, error) {
	ctx = context.WithValue(ctx, goa.MethodKey, "osmoPools")
	ctx = context.WithValue(ctx, goa.ServiceKey, "sdk-utilities")
	resp, err := s.OsmoPoolsH.Handle(ctx, message)
	if err != nil {
		return nil, goagrpc.EncodeError(err)
	}
	return resp.(*sdk_utilitiespb.OsmoPoolsResponse), nil
}

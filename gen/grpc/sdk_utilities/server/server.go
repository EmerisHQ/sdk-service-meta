...
type Server struct {
	...
	IbcChannelClientStateH goagrpc.UnaryHandler
...
func New(e *sdkutilities.Endpoints, uh goagrpc.UnaryHandler) *Server {
	return &Server{
		...
		IbcChannelClientStateH: NewIbcChannelClientStateHandler(e.IbcChannelClientState, uh),
	}
}
...
// NewIbcChannelClientStateHandler creates a gRPC handler which serves the
// "sdk-utilities" service "ibcChannelClientState" endpoint.
func NewIbcChannelClientStateHandler(endpoint goa.Endpoint, h goagrpc.UnaryHandler) goagrpc.UnaryHandler {
	if h == nil {
		h = goagrpc.NewUnaryHandler(endpoint, DecodeIbcChannelClientStateRequest, EncodeIbcChannelClientStateResponse)
	}
	return h
}

// IbcChannelClientState implements the "IbcChannelClientState" method in
// sdk_utilitiespb.SdkUtilitiesServer interface.
func (s *Server) IbcChannelClientState(ctx context.Context, message *sdk_utilitiespb.IbcChannelClientStateRequest) (*sdk_utilitiespb.IbcChannelClientStateResponse, error) {
	ctx = context.WithValue(ctx, goa.MethodKey, "ibcChannelClientState")
	ctx = context.WithValue(ctx, goa.ServiceKey, "sdk-utilities")
	resp, err := s.IbcChannelClientStateH.Handle(ctx, message)
	if err != nil {
		return nil, goagrpc.EncodeError(err)
	}
	return resp.(*sdk_utilitiespb.IbcChannelClientStateResponse), nil
}

...
type Server struct {
	...
	TxFeeEstimateH       goagrpc.UnaryHandler
...
func New(e *sdkutilities.Endpoints, uh goagrpc.UnaryHandler) *Server {
	return &Server{
		...
		TxFeeEstimateH:       NewTxFeeEstimateHandler(e.TxFeeEstimate, uh),
	}
}
...
// NewTxFeeEstimateHandler creates a gRPC handler which serves the
// "sdk-utilities" service "txFeeEstimate" endpoint.
func NewTxFeeEstimateHandler(endpoint goa.Endpoint, h goagrpc.UnaryHandler) goagrpc.UnaryHandler {
	if h == nil {
		h = goagrpc.NewUnaryHandler(endpoint, DecodeTxFeeEstimateRequest, EncodeTxFeeEstimateResponse)
	}
	return h
}

// TxFeeEstimate implements the "TxFeeEstimate" method in
// sdk_utilitiespb.SdkUtilitiesServer interface.
func (s *Server) TxFeeEstimate(ctx context.Context, message *sdk_utilitiespb.TxFeeEstimateRequest) (*sdk_utilitiespb.TxFeeEstimateResponse, error) {
	ctx = context.WithValue(ctx, goa.MethodKey, "txFeeEstimate")
	ctx = context.WithValue(ctx, goa.ServiceKey, "sdk-utilities")
	resp, err := s.TxFeeEstimateH.Handle(ctx, message)
	if err != nil {
		return nil, goagrpc.EncodeError(err)
	}
	return resp.(*sdk_utilitiespb.TxFeeEstimateResponse), nil
}
